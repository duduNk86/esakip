...
class ChartStatistikPenilaianOpd extends Component
{
    public $chartLabels = [];
    public $chartValues = [];
        ...
        $latestPeriode = Periode::whereHas('penilaianOpd', function ($query) {
            $query->where('status', 'Pengumuman')
                ->whereHas('opd', function ($q) {
                    $q->where('nama_opd', 'NOT LIKE', '%Kecamatan%');
                });
        })
            ->orderBy('tahun', 'desc')
            ->first();
    ...
    public function loadChartData($tahun = null)
    {
        ...
        $penilaian = Penilaian_opd::with('periode', 'opd')
            ...
            ->where('opds.nama_opd', 'NOT LIKE', '%Kecamatan%')
        ...
        $this->dispatch('chartOpdDataUpdated', [
    ...
    {
        return view('livewire.dashboard.chart-statistik-penilaian-opd', [
